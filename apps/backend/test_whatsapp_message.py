...
async def test_whatsapp_messaging():
    ...
    connection_id = UUID('9b1861fc-79e1-40cd-91cd-959717124fff')  # Active connection with credentials
    ...
    async for db in get_db():
        try:
            ...
            if not target_room:
                ...
                if rooms:
                    target_room = rooms[0]
                    print(f"   Using first available room for testing: {target_room['room_id']}")
                else:
            ...
            if db_messages:
                ...
                for i, msg in enumerate(db_messages[:5], 1):
                    ...
                    print(f"      From: {msg.sender.full_name if msg.sender else 'Unknown'}")
