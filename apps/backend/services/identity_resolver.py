...
class IdentityResolver:
    ...
    async def link_identities(
        self, contact_id: UUID, platform: str, platform_user_id: str
    ) -> Contact:
        """
    ...
    async def find_cross_platform_matches(
        self, contact_id: UUID, min_confidence: float = MEDIUM_CONFIDENCE
    ...
    def _calculate_identity_confidence(
        ...
        name_similarity = self._calculate_name_similarity(
            contact1.canonical_name, contact2.canonical_name
        )
        ...
        if contact1.platform_identities and contact2.platform_identities:
            platform_overlap = set(contact1.platform_identities.keys()) & set(
                contact2.platform_identities.keys()
            )
            if platform_overlap:
    ...
    async def merge_contacts(
        self, primary_contact_id: UUID, secondary_contact_id: UUID
    ) -> Contact:
        """
        ...
        thread_stmt = select(Thread).where(Thread.contact_id == secondary_contact_id)
    ...
    async def split_contact(
        ...
        for participant in participants:
            ...
            if participant.platform not in new_contact.platform_identities:
                new_contact.platform_identities[participant.platform] = (
        ...
        thread_stmt = select(Thread).where(Thread.contact_id == contact_id)
        thread_result = await self.session.execute(thread_stmt)
        threads = thread_result.scalars().all()
    ...
    async def get_contact_identity_summary(self, contact_id: UUID) -> Dict:
        """
