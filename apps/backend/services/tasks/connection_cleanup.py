...
async def cleanup_inactive_connections(
    ...
    try:
        ...
        if dry_run:
            logger.info(f"DRY RUN: Would delete {len(stale_connections)} inactive connections")
            return stats
        ...
        if stale_connections:
            ...
            logger.info(f"Deleted {stats['deleted']} inactive connections older than {max_age_hours}h")
...
async def cleanup_revoked_connections(
    ...
    try:
        ...
        if dry_run:
            logger.info(f"DRY RUN: Would delete {len(old_revoked)} old revoked connections")
            return stats
        ...
        if old_revoked:
            ...
            stats["deleted"] = result.rowcount
            logger.info(f"Deleted {stats['deleted']} revoked connections older than {max_age_days} days")
...
async def run_daily_cleanup(db: AsyncSession) -> dict:
    ...
    try:
        ...
        total_deleted = (
            results["inactive_cleanup"].get("deleted", 0) +
            results["revoked_cleanup"].get("deleted", 0)
        )
