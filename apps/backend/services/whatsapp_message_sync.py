...
from integrations.whatsapp_connector import WhatsAppConnector, MatrixMessage
...
class WhatsAppMessageSyncService:
            ...
            try:
                ...
                for room_info in rooms:
                    ...
                    try:
                        ...
                        self.logger.info(f"Synced {synced_count}/{len(matrix_messages)} messages from room {room_id}")
    ...
    async def _sync_room_messages(
        ...
        for msg_event in matrix_messages:
            try:
                ...
                client = WhatsAppConnector(connection.id, connection.credentials)._get_client()
                ...
                existing = await self._find_existing_message(db, connection.id, matrix_msg.event_id)
                if existing:
                    continue
                
                # Create raw message
                raw_message = await self._create_raw_message(db, connection, matrix_msg, msg_event)
                
                # Create normalized message
                await self._create_normalized_message(db, connection, raw_message, matrix_msg, room_id)
                
                synced_count += 1
                
            except Exception as e:
                self.logger.error(f"Failed to sync message {msg_event.get('event_id', 'unknown')}: {e}")
                continue
    ...
    async def _create_normalized_message(
        ...
        sender = await self._get_or_create_participant(db, connection, matrix_msg.sender)
    ...
    async def _get_or_create_participant(
        ...
        try:
            ...
            self.logger.error(f"Failed to get/create participant for {matrix_sender}: {e}")
            return None
