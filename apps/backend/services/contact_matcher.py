...
class ContactMatcher:
    ...
    async def match_participant(
        self, participant: Participant, auto_create: bool = True
    ...
    async def _match_by_email(self, email: str) -> Tuple[Optional[Contact], float]:
        """
        ...
        stmt = select(Contact).where(Contact.emails.contains([normalized_email]))
        result = await self.session.execute(stmt)
        contact = result.scalar_one_or_none()

        if contact:
            logger.info(f"Matched contact {contact.id} by email: {normalized_email}")
            return contact, self.EMAIL_MATCH_CONFIDENCE

        return None, 0.0

    async def _match_by_phone(self, phone: str) -> Tuple[Optional[Contact], float]:
        """
        ...
        stmt = select(Contact).where(Contact.phones.contains([normalized_phone]))
        result = await self.session.execute(stmt)
        contact = result.scalar_one_or_none()

        if contact:
            logger.info(f"Matched contact {contact.id} by phone: {normalized_phone}")
            return contact, self.PHONE_MATCH_CONFIDENCE

        return None, 0.0

    async def _match_by_name(self, name: str) -> Tuple[Optional[Contact], float]:
        """
        ...
        for contact in contacts:
            similarity = self._calculate_name_similarity(
                normalized_name, contact.canonical_name.lower().strip()
            )
    ...
    async def _create_contact_from_participant(
        ...
        platform_identities = {participant.platform: participant.platform_user_id}
        ...
        await self._emit_contact_event(EventType.CONTACT_CREATED, contact)
    ...
    async def _link_participant_to_contact(
        self, participant: Participant, contact: Contact
    ) -> None:
        """
        ...
        logger.info(f"Linked participant {participant.id} to contact {contact.id}")

    async def _merge_participant_info(
        self, contact: Contact, participant: Participant
    ) -> None:
        """
        Merge participant information into contact.
        ...
        if participant.platform not in contact.platform_identities:
            contact.platform_identities[participant.platform] = (
    ...
    async def match_all_unmatched_participants(
        ...
        for participant in participants:
            try:
                ...
                logger.error(f"Failed to match participant {participant.id}: {e}")
                continue
    ...
    async def get_potential_matches(
        self, participant: Participant, min_confidence: float = 0.5
    ...
    async def _emit_contact_event(
        self, event_type: EventType, contact: Contact, extra_data: dict = None
    ) -> None:
        """
