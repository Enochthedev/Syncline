...
from integrations.whatsapp_connector import BridgeStatus, WhatsAppConnector
...
class WhatsAppSessionManager:
    ...
    async def create_session(
        self, db: AsyncSession, connection_id: UUID, user_id: UUID
    ) -> WhatsAppSession:
        """
    ...
    async def login_with_phone(
        self, db: AsyncSession, connection_id: UUID, phone_number: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"success": False, "message": f"Phone login failed: {str(e)}"}
            ...
            if not session:
                ...
                if connection:
                    session = await self.create_session(
                        db, connection_id, connection.user_id
                    )
                else:
            ...
            try:
                ...
                for attempt in range(3):
                    try:
                        ...
                        self.logger.warning(
                            f"Bridge status attempt {attempt + 1} failed: {e}"
                        )
                        if attempt < 2:
                ...
                return {
                    ...
                    "session_age": (
                        (datetime.utcnow() - session.session_created_at).total_seconds()
    ...
    async def logout(self, db: AsyncSession, connection_id: UUID) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if not logout_success:
                    try:
                        ...
                        disconnect_response = await client.send_bridge_command(
                            "disconnect"
                        )
                ...
                if not logout_success:
                    try:
                        ...
                        for cmd in ["logout", "logout-matrix", "delete-session"]:
                            try:
                                await client.send_bridge_command(
                                    cmd, wait_for_response=False
                                )
                ...
                return {
                    ...
                    "message": (
                        "Logged out from WhatsApp successfully"
                        if logout_success
    ...
    async def cleanup_session(self, db: AsyncSession, connection_id: UUID) -> bool:
        """
    ...
    async def _update_connection_status(
        self, db: AsyncSession, connection_id: UUID, status: str
    ) -> None:
    ...
    async def cleanup_data(
        ...
        try:
            ...
            return {"success": True, "messages_deleted": 0, "sessions_cleared": 1}
        except Exception as e:
    ...
    async def check_login_completion(
        ...
        try:
            ...
            return {"success": False, "message": f"Check failed: {str(e)}"}
