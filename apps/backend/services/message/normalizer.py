...
class MessageNormalizer:
    ...
    def normalize_message(
        self, raw_message: RawMessage, connection_id: UUID
    ) -> UnifiedMessage:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to normalize message {raw_message.id}: {e}", exc_info=True
            )
            raise MessageNormalizationError(
    ...
    def get_stats(self) -> Dict:
        ...
        total_messages = (
            self.stats["messages_normalized"] + self.stats["normalization_errors"]
        )
