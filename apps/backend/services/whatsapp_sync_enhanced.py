...
class WhatsAppSyncEnhanced:
    ...
    async def sync_messages_enhanced(
        ...
        async with self._active_syncs[connection_id]:
            ...
            for attempt in range(1, max_retries + 1):
                stats.add_attempt()
                logger.info(
                    f"Sync attempt {attempt}/{max_retries} for connection {connection_id}"
                )

                try:
                    # Execute sync attempt
                    result = await self._execute_sync(
                        connection_id, limit, force, stats
                    )
    ...
    async def _execute_sync(
        self, connection_id: UUID, limit: int, force: bool, stats: SyncStats
    ) -> Dict[str, Any]:
                    ...
                    try:
                        ...
                        for message_data in messages:
                            try:
                                # Normalize message
                                normalized = (
                                    await self.message_normalizer.normalize_message(
    ...
    async def get_sync_status(self, connection_id: UUID) -> Dict[str, Any]:
        ...
        async with get_session() as session:
            ...
            is_syncing = (
                connection_id in self._active_syncs
                and self._active_syncs[connection_id].locked()
            )
    ...
    async def discover_rooms_enhanced(
        self, connection_id: UUID, wait_time: int = 10, max_attempts: int = 3
    ) -> Dict[str, Any]:
        """
