...
class ConsumerGroupManager:
    ...
    async def create_group(
        self, stream: str, group: str, start_id: str = "0", mkstream: bool = True
    ) -> bool:
        """
        ...
        try:
            ...
            if "BUSYGROUP" in str(e):
                self.logger.debug(
                    f"Consumer group '{group}' already exists for '{stream}'"
                )
                return False
            else:
    ...
    async def delete_group(self, stream: str, group: str) -> bool:
        ...
        try:
            ...
            if result:
                self.logger.info(
                    f"Deleted consumer group '{group}' from stream '{stream}'"
                )
            return bool(result)
        except ResponseError as e:
    ...
    def update_consumer_stats(
        self, consumer_id: str, messages_processed: int = 0, errors: int = 0
    ) -> None:
        """
    ...
    def get_active_consumers(self, group_name: str | None = None) -> list[ConsumerInfo]:
        ...
        """
        consumers = [c for c in self.consumers.values() if c.is_active]
        ...
        try:
            if consumer:
                ...
                pending = await self.redis.xpending_range(
                    stream, group, min="-", max="+", count=100, consumername=consumer
                )
            else:
    ...
    async def claim_pending_messages(
        self, stream: str, group: str, consumer: str, min_idle_time: int = 60000
    ) -> list:
        """
