...
class DeadLetterQueue:
    ...
    async def initialize(self) -> None:
        ...
        try:
            await self.redis.xgroup_create(
                name=self.DLQ_STREAM, groupname=self.DLQ_GROUP, id="0", mkstream=True
            )
    ...
    async def get_failed_events(
        ...
        try:
            ...
            messages = await self.redis.xrange(
                self.DLQ_STREAM, min="-", max="+", count=count
            )
    ...
    async def retry_event(
        self, event: Event, handler: Callable[[Event], Coroutine], retry_count: int = 0
    ) -> bool:
        """
    ...
    async def reprocess_failed_event(
        self, event_id: str, handler: Callable[[Event], Coroutine]
    ) -> bool:
        """
