...
class WhatsAppConnectionManager:
    ...
    async def _batch_test_connections(
        ...
        results = await asyncio.gather(
            *[test_single(conn) for conn in connections], return_exceptions=True
        )
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                self.logger.warning(
                    f"Batch test failed for connection {connections[i].id}: {result}"
                )
    ...
    async def get_or_create_connection(
        ...
        try:
            ...
            valid_active_connections = [
                conn for conn in active_connections if self._has_valid_credentials(conn)
            ]

            # Batch test all valid active connections in parallel (fixes N+1 problem)
            if valid_active_connections:
                status_map = await self._batch_test_connections(
                    valid_active_connections
                )
                ...
                for conn in valid_active_connections:
                    ...
                    if status.get("is_logged_in"):
                        ...
                        await self._update_connection_status(
                            db, conn.id, ConnectionStatus.INACTIVE
                        )
            ...
            for conn in inactive_connections[:3]:  # Check last 3 inactive connections
                if self._has_valid_credentials(conn):
                    self.logger.info(
                        f"Reusing inactive connection with credentials: {conn.id}"
                    )
                    return {
                        "connection_id": str(conn.id),
                        "is_existing": True,
    ...
    async def _test_connection(self, connection: PlatformConnection) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"is_logged_in": False, "phone": None, "connected": False}
    ...
    async def _update_connection_status(
        self, db: AsyncSession, connection_id: UUID, status: ConnectionStatus
    ) -> None:
        ...
        await db.execute(
            ...
            .values(
                status=status,
                last_sync_at=(
                    datetime.utcnow() if status == ConnectionStatus.ACTIVE else None
                ),
            )
        )
        await db.commit()
    ...
    async def _cleanup_old_connections(
        self, db: AsyncSession, user_id: UUID, keep_recent: int = 2
    ) -> None:
        ...
        try:
            ...
            if to_delete:
                await db.commit()
                self.logger.info(
                    f"Cleaned up {len(to_delete)} old WhatsApp connections for user {user_id}"
                )
