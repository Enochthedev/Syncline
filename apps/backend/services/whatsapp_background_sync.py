...
class WhatsAppBackgroundSyncService:
    ...
    async def start_background_sync(self, sync_interval: int = 300) -> None:
        ...
        self._running = True
        self.logger.info(f"Starting WhatsApp background sync (interval: {sync_interval}s)")
    ...
    async def _sync_all_active_connections(self) -> None:
        ...
        async for db in get_db():
            try:
                ...
                self.logger.info(f"Found {len(connections)} active WhatsApp connections")
    ...
    async def _sync_connection(self, db: AsyncSession, connection: PlatformConnection) -> None:
