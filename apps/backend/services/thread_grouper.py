...
class ThreadGrouper:
    ...
    async def group_message(
        self, message: Message, contact_id: UUID | None = None
    ) -> Thread:
        """
        ...
        if not message.thread_id:
            logger.warning(f"Message {message.id} has no thread_id, cannot group")
        ...
        recipients = (
            message.message_metadata.get("recipients", [])
        ...
        for recipient in recipients:
            if isinstance(recipient, dict) and "participant_id" in recipient:
                await self._add_participant_to_thread(
                    thread, UUID(recipient["participant_id"])
                )
    ...
    async def _find_or_create_thread(
        self, platform: str, platform_thread_id: str, contact_id: UUID | None = None
    ) -> Thread:
        """
        ...
        stmt = select(Thread).where(
            Thread.platform == platform, Thread.platform_thread_id == platform_thread_id
        )
        ...
        if thread:
            ...
            if contact_id and thread.contact_id != contact_id:
                ...
                await self._emit_thread_event(EventType.THREAD_UPDATED, thread)
        ...
        await self._emit_thread_event(EventType.THREAD_CREATED, thread)
    ...
    async def _update_thread_stats(self, thread: Thread, message: Message) -> None:
        """
    ...
    async def _add_participant_to_thread(
        self, thread: Thread, participant_id: UUID
    ) -> None:
        """
    ...
    async def associate_thread_with_contact(
        self, thread_id: UUID, contact_id: UUID
    ) -> Thread:
        """
        ...
        logger.info(f"Associated thread {thread_id} with contact {contact_id}")

        return thread

    async def get_thread_participants(self, thread_id: UUID) -> List[Participant]:
        """
    ...
    async def get_thread_by_platform_id(
        self, platform: str, platform_thread_id: str
    ) -> Thread | None:
        """
    ...
    async def get_threads_for_contact(
        self, contact_id: UUID, limit: int = 100, offset: int = 0
    ) -> List[Thread]:
        """
    ...
    async def recalculate_thread_stats(self, thread_id: UUID) -> Thread:
        """
                ...
                if msg.message_metadata:
                    ...
                    for recipient in recipients:
                        if (
                            ...
                            and "participant_id" in recipient
                        ):
    ...
    async def _emit_thread_event(self, event_type: EventType, thread: Thread) -> None:
        """
        ...
        try:
            event = ThreadEvent(
                ...
                payload={
                    ...
                    "participant_ids": [
                        str(pid) for pid in (thread.participant_ids or [])
                    ],
