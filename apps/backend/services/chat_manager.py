...
class ChatManagerService:
    ...
    async def get_chat_threads(
        ...
        try:
            ...
            if platform:
                ...
                platform_filter = (
                    platform.upper() if isinstance(platform, str) else platform
                )
            ...
            for msg in messages:
                ...
                if (
                    ...
                    and msg.message_metadata.get("sender_matrix_id")
                    == "@whatsappbot:localhost"
                    and "Hello, I'm a WhatsApp bridge bot"
                    in (msg.content.get("text", "") if msg.content else "")
                ):
                    continue
                ...
                if not thread_id:
                    ...
                    sender_info = (
                        msg.message_metadata.get("sender_matrix_id", "")
                        if msg.message_metadata
                        else ""
                    )
                    if sender_info:
                ...
                if thread_id not in thread_map:
                    ...
                    if msg.message_metadata:
                        sender_name = msg.message_metadata.get(
                    ...
                    thread = ChatThread(
                        ...
                        last_message=(
                            msg.content.get("text", "")[:100] if msg.content else ""
                        ),
            ...
            threads.sort(
                key=lambda t: t.last_message_time or datetime.min, reverse=True
            )
            ...
            self.logger.info(
                f"Retrieved {len(threads)} chat threads for user {user_id}"
            )
            return threads
    ...
    async def get_chat_messages(
        ...
        try:
            ...
            query = (
                ...
                )  # Always use uppercase for DB
                .options(
                    selectinload(Message.sender), selectinload(Message.attachments)
                )
                .order_by(desc(Message.timestamp))
                .limit(limit)
            )
            ...
            if thread_id.startswith(f"{platform}_"):
                ...
                query = query.where(
                    or_(
                        ...
                        Message.message_metadata["sender_matrix_id"].astext
                        == sender_part,
                    )
                )
            else:
            ...
            for msg in messages:
                ...
                chat_msg = ChatMessage(
                    ...
                    has_attachments=(
                        len(msg.attachments) > 0 if msg.attachments else False
                    ),
                )
            ...
            self.logger.info(
                f"Retrieved {len(chat_messages)} messages for thread {thread_id}"
            )
    ...
    async def _find_linked_contact(
        self, db: AsyncSession, message: Message, user_id: UUID
    ) -> Optional[Contact]:
        ...
        try:
            ...
            if message.sender and message.sender.contact_id:
                contact_query = select(Contact).where(
                    Contact.id == message.sender.contact_id
                )
                result = await db.execute(contact_query)
            ...
            if message.message_metadata:
                ...
                if phone:
                    contact_query = select(Contact).where(
                        and_(
                            Contact.user_id == user_id, Contact.phones.contains([phone])
                        )
                    )
                    result = await db.execute(contact_query)
                    return result.scalar_one_or_none()
    ...
    async def cleanup_bot_messages(
        self, db: AsyncSession, user_id: UUID, platform: str = "WHATSAPP"
    ) -> Dict[str, int]:
        """
        ...
        try:
                    ...
                    and_(
                        ...
                        or_(
                            ...
                            and_(
                                Message.message_metadata["sender_matrix_id"].astext
                                == "@whatsappbot:localhost",
                                Message.content["text"].astext.like(
                            ...
                            and_(
                                Message.message_metadata["sender_matrix_id"].astext
                                == "@syncline:localhost",
                                Message.content["text"].astext.in_(
                                    ["login qr", "login", "help", "status"]
                                ),
                            ),
                        ),
                    )
            ...
            self.logger.info(
                f"Cleaned up {deleted_count} bot messages for user {user_id}"
            )

            return {"deleted_messages": deleted_count, "platform": platform}

        except Exception as e:
            await db.rollback()
            self.logger.error(f"Failed to cleanup bot messages: {e}")
            return {"deleted_messages": 0, "platform": platform, "error": str(e)}
