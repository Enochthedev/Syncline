...
class MemoryManager:
    ...
    async def extract_memories_from_message(
        ...
        try:
            ...
            if auto_store and extraction_result.memories:
                ...
                logger.info(f"Stored {len(stored_memories)} memories from message {message.id}")
        ...
        try:
            ...
            search_results = await self.chroma_client.search(
                ...
                metadata_filter={
                    ...
                    "platform": platform,
                } if contact_id or platform else None,
            )
    ...
    def _create_memory_snippet(self, content: str, query: str, max_length: int = 200) -> str:
    ...
    async def run_maintenance(
        ...
        try:
            if apply_decay:
                results["decayed_memories"] = await self.proactive_agent.apply_memory_decay(db)
            
            if consolidate_memories:
                results["consolidated_memories"] = await self.proactive_agent.consolidate_memories(db)
    ...
    async def get_memory_stats(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            # Memories by type
            type_query = select(Memory.type, func.count(Memory.id)).where(
                Memory.is_active == True
            ...
            importance_query = select(Memory.importance, func.count(Memory.id)).where(
                Memory.is_active == True
