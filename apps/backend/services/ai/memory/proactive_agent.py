...
class MemoryContext(BaseModel):
    ...
    time_window: Optional[Tuple[datetime, datetime]] = Field(None, description="Time window")
...
class ProactiveMemoryAgent:
        ...
        for recall_type in recall_types:
            try:
                if recall_type == MemoryRecallType.CONTEXTUAL:
                    memories = await self._recall_contextual(db, context, limit // len(recall_types))
                elif recall_type == MemoryRecallType.TEMPORAL:
                    memories = await self._recall_temporal(db, context, limit // len(recall_types))
                elif recall_type == MemoryRecallType.SEMANTIC:
                    memories = await self._recall_semantic(db, context, limit // len(recall_types))
                elif recall_type == MemoryRecallType.COMMITMENT:
                    memories = await self._recall_commitments(db, context, limit // len(recall_types))
                elif recall_type == MemoryRecallType.RELATIONSHIP:
                    memories = await self._recall_relationship(db, context, limit // len(recall_types))
                else:
                    continue
    ...
    async def _recall_contextual(
        ...
        if context.current_platform:
            query = query.where(Memory.memory_metadata.op('->>')('platform') == context.current_platform)
    ...
    async def _recall_semantic(
        ...
        try:
            ...
            memory_ids = [UUID(result.metadata.get("memory_id")) for result in search_results]
    ...
    async def _recall_commitments(
        ...
        query = select(Memory).where(
            and_(
                ...
                Memory.importance.in_([MemoryImportance.HIGH, MemoryImportance.VERY_HIGH]),
            )
        )
        ...
        query = query.where(
            or_(
                ...
                and_(
                    ...
                    func.cast(Memory.memory_metadata.op('->>')('due_date'), db.DateTime) < now,
                ),
                # Upcoming commitments (next 7 days)
                and_(
                    Memory.memory_metadata.op('->>')('due_date').isnot(None),
                    func.cast(Memory.memory_metadata.op('->>')('due_date'), db.DateTime) <= now + timedelta(days=7),
                ),
            )
        )
    ...
    async def _recall_relationship(
        ...
        query = query.order_by(desc(Memory.importance), desc(Memory.last_accessed_at)).limit(limit)
    ...
    async def _calculate_memory_relevance(
        ...
        if context.current_platform:
            memory_platform = memory.memory_metadata.get("platform") if memory.memory_metadata else None
        ...
        if context.keywords:
            ...
            keyword_matches = sum(1 for keyword in context.keywords if keyword.lower() in content_lower)
    ...
    async def apply_memory_decay(
        ...
        try:
            ...
            for memory in memories:
                ...
                decay_amount = min(1, (days_since_access / self.memory_decay_days) * decay_factor)
    ...
    async def consolidate_memories(
        ...
        try:
            ...
            query = select(Memory).where(
                and_(
                    Memory.is_active == True,
                    Memory.importance != MemoryImportance.VERY_HIGH,  # Don't consolidate very important memories
                )
            )
                ...
                if len(similar_memories) >= self.consolidation_threshold:
                    ...
                    if consolidated_memory:
                        ...
                        for memory in similar_memories:
                            ...
                            memory.memory_metadata["consolidated_into"] = str(consolidated_memory.id)
                            memory.updated_at = datetime.utcnow()
    ...
    async def _calculate_memory_similarity(
        ...
        try:
            ...
            common_words = set(memory1.content.lower().split()) & set(memory2.content.lower().split())
            total_words = len(set(memory1.content.lower().split()) | set(memory2.content.lower().split()))
            if total_words > 0:
    ...
    async def _create_consolidated_memory(
        ...
        try:
            ...
            memory_contents = [f"Memory {i+1}: {m.content}" for i, m in enumerate(memories)]
            ...
            consolidated_content = await self.llm_provider.generate(prompt, config=config)
            ...
            logger.info(f"Created consolidated memory {consolidated_memory.id} from {len(memories)} memories")
    ...
    async def generate_recommendations(
        ...
        try:
            ...
            recommendations.extend(await self._generate_commitment_recommendations(memories))
            recommendations.extend(await self._generate_follow_up_recommendations(memories))
            recommendations.extend(await self._generate_relationship_recommendations(memories))
            recommendations.extend(await self._generate_context_recommendations(memories, context))
                ...
                try:
                    ...
                    if due_date < now:
                        ...
                        recommendations.append(MemoryRecommendation(
                            ...
                            description=memory.content[:200] + "..." if len(memory.content) > 200 else memory.content,
                            priority=0.9,
            ...
            if len(contact_mems) >= 3:  # Significant relationship
                ...
                if days_since >= 14:  # Haven't interacted in 2 weeks
                    recommendations.append(MemoryRecommendation(
                        ...
                        priority=0.5,
                        memory_ids=[m.id for m in contact_mems[-3:]],  # Last 3 memories
    ...
    async def _generate_context_recommendations(
        ...
        if context.current_contact and context.keywords:
            relevant_memories = [
                m for m in memories
                if m.contact_id == context.current_contact and
                any(keyword.lower() in m.content.lower() for keyword in context.keywords)
            ]
