...
from services.vector_db.chroma_client import ChromaDBClient, get_chroma_client, SearchResult
...
class SearchFilter(BaseModel):
    ...
    sender_ids: Optional[list[UUID]] = Field(default=None, description="Filter by sender IDs")
...
class SemanticSearchEngine:
    ...
    def _build_chroma_filter(self, filters: Optional[SearchFilter]) -> Optional[dict]:
        ...
        if filters.platforms:
            where["platform"] = filters.platforms[0] if len(filters.platforms) == 1 else {"$in": filters.platforms}
        if filters.thread_ids:
            where["thread_id"] = filters.thread_ids[0] if len(filters.thread_ids) == 1 else {"$in": filters.thread_ids}
    ...
    async def _fetch_messages(
        self, db: AsyncSession, message_ids: list[UUID], filters: Optional[SearchFilter] = None
    ) -> dict[UUID, Message]:
    ...
    def _combine_results(
        self, chroma_results: list[SearchResult], messages: dict[UUID, Message],
        filters: Optional[SearchFilter] = None
    ) -> list[MessageSearchResult]:
        """Combine ChromaDB results with message details."""
        combined = []
        ...
        for cr in chroma_results:
            ...
            msg = messages[msg_id]
            combined.append(MessageSearchResult(
                message_id=msg.id,
