...
class PIIRedactionService:
    ...
    def __init__(
        ...
        self.confidence_threshold = (
            confidence_threshold or settings.PII_CONFIDENCE_THRESHOLD
        )
        self.redaction_placeholder = (
            redaction_placeholder or settings.PII_REDACTION_PLACEHOLDER
        )
        self.enabled = (
            enabled if enabled is not None else settings.PII_REDACTION_ENABLED
        )
        ...
        if self._analyzer is None:
            try:
                ...
                nlp_configuration = {
                    ...
                    "models": [
                        {"lang_code": self.language, "model_name": "en_core_web_sm"}
                    ],
                }
    ...
    def redact_pii_batch(
        ...
        try:
            ...
            logger.info(f"Redacted {total_pii} PII entities from {len(texts)} texts")
            return results
