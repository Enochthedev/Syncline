...
class Settings(BaseSettings):
    ...
    # =============================================================================
    DEFAULT_LLM_PROVIDER: str = Field(default="openrouter", description="Primary LLM provider")
    DEFAULT_CHAT_MODEL: str = Field(default="anthropic/claude-3.5-sonnet", description="Default chat model")
    ...
    OPENROUTER_TIMEOUT: int = Field(default=120, description="OpenRouter request timeout")
    ...
    MATRIX_ACCESS_TOKEN: str = Field(
        default="",
        description="Matrix access token for Syncline user"
    )
    MATRIX_USER_ID: str = Field(
        default="@syncline:localhost",
        description="Matrix user ID for Syncline"
    )
    ...
    WHATSAPP_BRIDGE_ENABLED: bool = Field(
        default=False,
        description="Whether WhatsApp bridge is enabled"
    )
    WHATSAPP_BRIDGE_BOT_ID: str = Field(
        default="@whatsappbot:localhost",
        description="WhatsApp bridge bot Matrix ID"
    )
    ...
    WHATSAPP_PROVISIONING_SECRET: str = Field(
        default="",
        description="WhatsApp bridge provisioning shared secret"
    )
