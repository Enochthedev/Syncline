    ...
    async for db in get_db():
        try:
            ...
            for conn in connections:
                has_creds = bool(
                    conn.credentials
                    and conn.credentials.get("matrix_access_token")
                    and conn.credentials.get("matrix_user_id")
                )
            ...
            old_inactive = [
                c
                for c in inactive_with_creds + inactive_without_creds
                if c.created_at < cutoff_date
            ]
            to_delete.extend(old_inactive)
            print(
                f"🗑️  Deleting {len(old_inactive)} old inactive connections (>1 day old)"
            )

            # Keep recent inactive connections with credentials as backup
            recent_inactive_with_creds = [
                c for c in inactive_with_creds if c.created_at >= cutoff_date
            ]
            print(
                f"💾 Keeping {len(recent_inactive_with_creds)} recent inactive connections with credentials as backup"
            )
            ...
            if to_delete:
                ...
                for conn in to_delete:
                    print(
                        f"  - Deleting {conn.id} (Status: {conn.status}, Created: {conn.created_at})"
                    )
            ...
            for conn in active_remaining:
                has_creds = bool(
                    conn.credentials and conn.credentials.get("matrix_access_token")
                )
