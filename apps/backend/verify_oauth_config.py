...
def check_env_var(var_name: str, required: bool = True) -> Tuple[bool, str]:
    ...
    placeholders = [
        "your-", "your_", "placeholder", "change-this", 
        "xxx", "XXXX", "example", "test-only"
    ]
...
def generate_oauth_urls():
    ...
    gmail_redirect_uri = os.getenv("GMAIL_REDIRECT_URI", "http://localhost:8000/api/v1/connections/callback/gmail")
    gmail_scopes = os.getenv("GMAIL_SCOPES", "https://www.googleapis.com/auth/gmail.readonly,https://www.googleapis.com/auth/gmail.modify")
    ...
    if gmail_client_id and "your-" not in gmail_client_id.lower():
        ...
        gmail_url = f"https://accounts.google.com/o/oauth2/v2/auth?{urlencode(gmail_params)}"
    ...
    linkedin_redirect_uri = os.getenv("LINKEDIN_REDIRECT_URI", "http://localhost:8000/api/v1/connections/callback/linkedin")
    linkedin_scopes = os.getenv("LINKEDIN_SCOPES", "r_liteprofile,r_emailaddress,w_member_social")
...
def main():
    ...
    parser.add_argument("--test-gmail", action="store_true", help="Test Gmail configuration")
    parser.add_argument("--test-linkedin", action="store_true", help="Test LinkedIn configuration")
    parser.add_argument("--all", action="store_true", help="Test all OAuth configurations")
    parser.add_argument("--urls", action="store_true", help="Generate OAuth authorization URLs")
    parser.add_argument("--setup", action="store_true", help="Print setup instructions")
    parser.add_argument("--connectivity", action="store_true", help="Test endpoint connectivity")
    
    args = parser.parse_args()
    
    # Default to --all if no specific options given
    if not any([args.test_gmail, args.test_linkedin, args.all, args.urls, args.setup, args.connectivity]):
        args.all = True
