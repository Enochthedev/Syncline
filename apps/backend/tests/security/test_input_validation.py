...
class TestInputValidationSecurity:
    ...
    def test_tc25_sql_injection_in_search(self, client, auth_headers):
        ...
        for payload in sql_injection_payloads:
            ...
            if response.status_code == 200:
                ...
                assert (
                    "syntax error" not in response_text
                ), f"SQL syntax error exposed: {payload}"
                assert (
                    "sql" not in response_text or "error" not in response_text
    ...
    def test_tc25_sql_injection_in_message_id(self, client, auth_headers):
        ...
        for injection_id in injection_ids:
            response = client.get(
                f"/api/v1/messages/{injection_id}", headers=auth_headers
            )
    ...
    def test_tc25_sql_injection_in_json_body(self, client, auth_headers):
        ...
        for payload in injection_payloads:
            response = client.post(
                "/api/v1/messages/search", headers=auth_headers, json=payload
            )
    ...
    def test_tc26_xss_in_message_content(self, client, auth_headers):
        ...
        for payload in xss_payloads:
            ...
            if create_response.status_code in [200, 201]:
                ...
                assert (
                    "onerror=" not in content and "onload=" not in content
    ...
    def test_tc26_xss_in_contact_name(self, client, auth_headers):
        ...
        response = client.post(
            "/api/v1/contacts",
            headers=auth_headers,
            json={"name": xss_name, "email": "test@example.com"},
        )
        ...
        if response.status_code in [200, 201]:
            ...
            assert "<script>" not in name, "XSS in contact name not escaped"
    ...
    def test_tc27_deep_json_nesting(self, client, auth_headers):
        ...
        try:
            response = client.post(
                "/api/v1/messages", headers=auth_headers, json=nested
            )
    ...
    def test_tc27_many_array_elements(self, client, auth_headers):
        ...
        large_array = {"items": list(range(1000000))}
...
class TestInputSanitization:
    ...
    def test_null_bytes_rejected(self, client, auth_headers):
        ...
        payload = {"content": "Test\x00Message", "sender": "test\x00user"}

        response = client.post("/api/v1/messages", headers=auth_headers, json=payload)
    ...
    def test_unicode_normalization(self, client, auth_headers):
        ...
        for payload in payloads:
            response = client.post(
                "/api/v1/messages", headers=auth_headers, json=payload
            )
    ...
    def test_path_traversal_blocked(self, client, auth_headers):
        ...
        for path in traversal_attempts:
            ...
            response = client.get(f"/api/v1/files/{path}", headers=auth_headers)
    ...
    def test_command_injection_blocked(self, client, auth_headers):
        ...
        for payload in injection_payloads:
            response = client.post(
                "/api/v1/messages", headers=auth_headers, json={"content": payload}
            )
