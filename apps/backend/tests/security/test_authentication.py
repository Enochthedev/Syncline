...
class TestAuthenticationSecurity:
    ...
    def test_tc20_forged_oauth_token_rejected(self, client):
        ...
        forged_token = jwt.encode(
            forged_payload, "fake_secret_key_not_real", algorithm="HS256"
        )

        # Attempt to access protected endpoint
        response = client.get(
            "/api/v1/messages", headers={"Authorization": f"Bearer {forged_token}"}
        )
    ...
    def test_tc21_expired_token_rejected(self, client):
        ...
        expired_token = jwt.encode(
            expired_payload, os.getenv("JWT_SECRET", "test_secret"), algorithm="HS256"
        )

        response = client.get(
            "/api/v1/messages", headers={"Authorization": f"Bearer {expired_token}"}
        )
    ...
    def test_tc21_very_old_token_rejected(self, client):
        ...
        old_token = jwt.encode(
            old_payload, os.getenv("JWT_SECRET", "test_secret"), algorithm="HS256"
        )

        response = client.get(
            "/api/v1/connections", headers={"Authorization": f"Bearer {old_token}"}
        )
...
class TestOAuthProviderIntegration:
    ...
    def test_oauth_callback_with_invalid_code_rejected(self, client):
        ...
        response = client.get(
            "/api/v1/auth/callback/google", params={"code": "invalid_auth_code_123"}
        )
    ...
    def test_oauth_state_parameter_validation(self, client):
        ...
        if init_response.status_code == 200:
            ...
            assert "state=" in auth_url or data.get(
                "state"
...
class TestSessionManagement:
    ...
    def test_logout_invalidates_session(self, client):
        ...
        assert response.status_code in [200, 204, 401], "Logout endpoint should respond"
