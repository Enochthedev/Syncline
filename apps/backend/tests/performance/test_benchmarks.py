    ...
    async def run_async_benchmark(
        ...
        result = BenchmarkResult(
            ...
            throughput_per_second=(
                (iterations / (total_time_ms / 1000)) if total_time_ms > 0 else 0
            ),
...
class TestDatabaseQueryPerformance:
    ...
    async def test_message_retrieval_by_id(self, runner, client):
        ...
        result = await runner.run_async_benchmark(
            name="message_retrieval_by_id", func=get_message, iterations=100
        )
    ...
    async def test_contact_profile_query(self, runner, client):
        ...
        result = await runner.run_async_benchmark(
            name="contact_profile_query", func=get_contact_profile, iterations=50
        )
    ...
    async def test_thread_retrieval(self, runner, client):
        ...
        result = await runner.run_async_benchmark(
            name="thread_retrieval", func=get_thread, iterations=50
        )
    ...
    async def test_message_insertion(self, runner, client):
        ...
        result = await runner.run_async_benchmark(
            name="message_insertion", func=insert_message, iterations=100
        )
...
class TestVectorSearchPerformance:
    ...
    async def test_vector_search_10k(self, runner, client):
        ...
        result = await runner.run_async_benchmark(
            name="vector_search_10k", func=vector_search, iterations=50
        )
    ...
    async def test_vector_search_100k(self, runner, client):
        ...
        result = await runner.run_async_benchmark(
            name="vector_search_100k", func=vector_search, iterations=30
        )
...
class TestEntityExtractionPerformance:
    ...
    async def test_entity_extraction_performance(self, runner, client):
        ...
        async def extract_entities():
            ...
            idx += 1
            await client.post("/api/v1/ai/extract-entities", json={"text": message})
        ...
        print(
            f"Expected throughput: 33 msgs/min, Actual: {throughput_per_minute:.1f} msgs/min"
        )
