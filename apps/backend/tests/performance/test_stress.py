    ...
    async def test_message_injection_throughput(self, sample_messages):
        ...
        async with httpx.AsyncClient(
            ...
            for batch_start in range(0, len(sample_messages), batch_size):
                ...
                print(f"\rProgress: {progress:.1f}% ({batch_start + len(batch)}/{len(sample_messages)})", end="")
        ...
        assert result.throughput_per_second > 15, f"Throughput below minimum: {result.throughput_per_second}"
        assert result.failed_operations == 0 or (result.failed_operations / result.total_operations) < 0.01
...
class TestRecoveryTime(StressTestBase):
    ...
    async def test_recovery_after_overload(self):
        ...
        async with httpx.AsyncClient(
            ...
            while not recovered and (time.perf_counter() - recovery_start) < max_recovery_time:
                try:
                    start = time.perf_counter()
        ...
        print(f"Final Latency: {recovery_latencies[-1]:.2f}ms" if recovery_latencies else "N/A")
        print(f"{'='*60}\n")
...
class TestEndurance:
    ...
    async def test_endurance_short(self):
        ...
        if metrics["response_times"]:
            print(f"Avg Response Time: {statistics.mean(metrics['response_times']):.2f}ms")
            print(f"Response Time Std Dev: {statistics.stdev(metrics['response_times']) if len(metrics['response_times']) > 1 else 0:.2f}ms")
        print(f"{'='*60}\n")
