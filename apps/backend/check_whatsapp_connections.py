        ...
        try:
            ...
            for i, conn in enumerate(connections, 1):
                ...
                if conn.credentials:
                    ...
                    if has_matrix:
                        print(
                            f"   Matrix homeserver: {conn.credentials.get('matrix_homeserver_url')}"
                        )
                        print(
                            f"   Matrix user: {conn.credentials.get('matrix_user_id')}"
                        )
                        print(
                            f"   Bridge bot: {conn.credentials.get('bridge_bot_id', 'Not set')}"
                        )
                else:
