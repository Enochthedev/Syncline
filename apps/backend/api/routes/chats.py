...
async def get_chat_threads(
    ...
    try:
        ...
        threads = await chat_service.get_chat_threads(
            db=db, user_id=user_id, platform=platform, limit=limit
        )

        logger.info(f"Retrieved {len(threads)} chat threads for user {user_id}")

        return ChatThreadsResponse(threads=threads, total=len(threads))
    ...
    try:
        ...
        if success:
            return LinkChatResponse(
                success=True, message=f"Successfully linked chat {thread_id} to contact"
            )
        else:
            return LinkChatResponse(
                success=False, message="Failed to link chat to contact"
            )
...
async def send_message(
    ...
    from db.models.connection import ConnectionStatus, PlatformConnection, PlatformType
    ...
    try:
        ...
        event_id = await connector.send_message(
            room_id=room_id, content=request.content
        )
...
async def cleanup_bot_messages(
    ...
    try:
        ...
        logger.info(
            f"Cleaned up {cleanup_result.get('deleted_messages', 0)} bot messages for platform {platform}"
        )
