...
from services.search.fulltext_search import FullTextSearchResult as FullTextResult
from services.search.fulltext_search import get_fulltext_search_service
...
class MessageDetailResponse(MessageResponse):
    ...
    raw_data: dict[str, Any] | None = Field(
        None, description="Original raw message data"
    )
...
class MessageStats(BaseModel):
    ...
    messages_by_platform: dict[str, int] = Field(
        ..., description="Message count by platform"
    )
...
async def search_messages(
    ...
    try:
        ...
        logger.info(
            f"Full-text search returned {len(results)} results (total: {total})"
        )
...
async def list_messages(
    ...
    query = select(Message).options(
        selectinload(Message.attachments), selectinload(Message.sender)
    )
...
async def get_message_stats(
    ...
    platform_query = select(
        Message.platform, func.count(Message.id).label("count")
    ...
    processing_query = select(
        ...
        .filter(RawMessage.processed == True)
        .label("processed"),
        func.count(RawMessage.id)
        .filter(RawMessage.processed == False)
        .label("pending"),
    )
...
async def get_message(
    ...
    query = query.options(
        selectinload(Message.attachments), selectinload(Message.raw_message)
    )
    ...
    # Extract sender info
    sender_info = (
        message.message_metadata.get("sender", {}) if message.message_metadata else {}
    )
