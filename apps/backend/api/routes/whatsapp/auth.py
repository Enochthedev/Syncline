...
async def get_session_status(
    ...
    if not status_result["success"]:
        raise HTTPException(
            status_code=500, detail=status_result.get("error", "Status check failed")
        )
...
@router.get(
    "/{connection_id}/login",
    response_model=WhatsAppLoginResponse,
    summary="Get QR Code",
)
async def get_whatsapp_qr_code(
    ...
    has_credentials = bool(
        ...
        and connection.credentials.get("matrix_access_token")
        and connection.credentials.get("matrix_user_id")
    )
    ...
    if not qr_result["success"]:
        raise HTTPException(
            status_code=500, detail=qr_result.get("error", "Failed to get QR code")
        )
...
async def login_with_phone(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get pairing code: {str(e)}"
        )
    finally:
