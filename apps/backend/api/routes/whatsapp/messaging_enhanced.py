...
class EnhancedSyncRequest(BaseModel):
    ...
    limit: int = Field(default=100, ge=1, le=1000, description="Maximum messages to sync per room")
    ...
    base_delay: float = Field(default=1.0, ge=0.1, le=10.0, description="Base delay between retries")

class RoomDiscoveryRequest(BaseModel):
    """Room discovery request parameters"""
    wait_time: int = Field(default=10, ge=1, le=60, description="Time to wait for rooms to appear")
    max_attempts: int = Field(default=3, ge=1, le=5, description="Maximum discovery attempts")

class BridgeSyncRequest(BaseModel):
    """Bridge sync request parameters"""
    timeout: int = Field(default=30, ge=5, le=120, description="Bridge operation timeout")
    force_reconnect: bool = Field(default=False, description="Force bridge reconnection")
...
async def get_whatsapp_connection(connection_id: UUID, user: User) -> PlatformConnection:
    ...
    async with get_session() as session:
        ...
        if not connection:
            raise HTTPException(
                status_code=404,
                detail=f"WhatsApp connection {connection_id} not found"
            )
...
async def discover_rooms_enhanced(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Room discovery failed: {str(e)}"
        )
...
async def sync_bridge(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Bridge sync failed: {str(e)}"
        )
...
async def get_diagnostics(
    ...
    try:
        ...
        if not diagnostics["bridge_connected"]:
            recommendations.append("Bridge connection failed - check Matrix homeserver and credentials")
        
        if not diagnostics["logged_in"]:
            recommendations.append("WhatsApp not logged in - scan QR code using /login endpoint")
        
        if diagnostics["rooms_count"] == 0:
            recommendations.append("No rooms found - try room discovery or check WhatsApp account")
        ...
        if diagnostics["last_error"]:
            if "auth" in diagnostics["last_error"].lower():
                recommendations.append("Authentication error - check Matrix access token")
...
async def start_background_sync(
    ...
    try:
        ...
        if sync_interval < 60:
            raise HTTPException(
                status_code=400,
                detail="Sync interval must be at least 60 seconds"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to start background sync: {str(e)}"
        )

async def _background_sync_task(connection_id: UUID, sync_interval: int):
    """Background sync task"""
    logger.info(f"Starting background sync for {connection_id} with {sync_interval}s interval")
    ...
    while True:
        try:
            ...
            if result["success"]:
                logger.info(f"Background sync completed: {result['total_messages_synced']} messages")
            else:
