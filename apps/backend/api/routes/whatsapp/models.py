...
class WhatsAppLoginResponse(BaseModel):
    ...
    pairing_code: Optional[str] = Field(
        None, description="Pairing code for phone login"
    )
    already_logged_in: bool = Field(
        default=False, description="Whether already logged in"
    )
...
class WhatsAppSessionResponse(BaseModel):
    ...
    session_age_seconds: Optional[int] = Field(
        None, description="Session age in seconds"
    )
