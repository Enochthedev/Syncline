...
async def login_with_phone_enhanced(
    ...
    try:
        ...
        logger.info(f"[PHONE_LOGIN] Using homeserver: {credentials['matrix_homeserver_url']}")
        ...
        try:
            ...
            if pairing_code and len(pairing_code) >= 8:
                ...
                # Unexpected response
                logger.error(f"[PHONE_LOGIN] Unexpected response from bridge: {pairing_code}")
                raise HTTPException(
                    status_code=500,
                    detail={
        ...
        for test in diagnostics["tests"]:
            if not test["success"]:
                if test["test"] == "credentials_configured":
                    ...
                    recommendations.append("Check if Matrix homeserver is running and accessible")
