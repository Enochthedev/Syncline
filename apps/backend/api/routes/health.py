...
class SystemStats(BaseModel):
    ...
    database: Dict[str, Any] = Field(
        default_factory=dict, description="Database statistics"
    )
...
async def detailed_health_check(
    ...
    redis_health = ComponentHealth(
        name="redis", status="unknown", message="Redis health check not implemented yet"
    )
...
async def database_health(
    ...
    if health.status == "unhealthy":
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=health.message
        )
...
async def stage_health(
    stage: str, db: AsyncSession = Depends(get_database_session)
) -> ComponentHealth:
    """
...
async def system_stats(db: AsyncSession = Depends(get_database_session)) -> SystemStats:
    """
...
async def cleanup_connections(
    db: AsyncSession = Depends(get_database_session), dry_run: bool = False
) -> CleanupResponse:
    """
    ...
    try:
        ...
        result.total_deleted = result.inactive_cleanup.get(
            "deleted", 0
...
async def stop_collection():
    ...
    return {"status": "stopped", "message": "Message collection worker stopped"}
