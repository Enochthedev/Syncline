...
class ThreadDetailResponse(ThreadResponse):
    ...
    recent_messages: list[MessageSummary] = Field(
        ...
        description="Recent messages (last 5)"
    )
...
async def list_threads(
    ...
    has_contact: bool | None = Query(None, description="Filter threads with/without contact"),
    min_messages: int | None = Query(None, description="Minimum message count"),
    start_date: datetime | None = Query(None, description="Filter threads after this date"),
    end_date: datetime | None = Query(None, description="Filter threads before this date"),
) -> ThreadListResponse:
    """
...
async def get_thread(
    ...
    # Get recent messages
    messages_query = select(Message).where(Message.thread_id == thread.platform_thread_id)
    ...
    for msg in messages:
        ...
        recent_messages.append(MessageSummary(
            id=msg.id,
