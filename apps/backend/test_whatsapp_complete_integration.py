    ...
    async for db in get_db():
        try:
            ...
            if conn_result.get("is_existing") and conn_result.get("is_logged_in"):
                print(
                    "   ✅ Connection management working - found existing logged-in connection"
                )
            else:
            ...
            try:
                ...
                is_logged_in = (
                    status.logged_in
                    if hasattr(status, "logged_in")
                    else status.get("logged_in", False)
                )
                ...
                if rooms:
                    ...
                    event_id = await connector.send_message(
                        test_room["room_id"], test_message
                    )
            ...
            manual_sync_result = await background_sync_service.sync_connection_now(
                connection_id
            )
            ...
            if webhook_process_result["success"]:
                ...
                print(
                    f"   Events processed: {webhook_process_result['events_processed']}"
                )
            else:
                print(
                    f"   ❌ Webhook processing failed: {webhook_process_result.get('error')}"
                )
            ...
            if tests_passed == total_tests:
                ...
                print(
                    f"⚠️  {total_tests - tests_passed} test(s) failed. Check the logs above."
                )

            # Mobile app integration notes
            print(f"\n📱 MOBILE APP INTEGRATION STATUS:")
            print(
                "✅ Connection check endpoint: /api/v1/whatsapp/connections/check-existing"
            )
            print("✅ Mobile app updated to check existing connections first")
            print(
                "✅ Background sync available: /api/v1/whatsapp/{id}/background-sync/start"
            )
