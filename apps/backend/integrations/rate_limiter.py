...
class RateLimitConfig(BaseModel):
    ...
    retry_after_seconds: int = Field(
        default=60,
        description="Default retry delay when rate limited"
    )
...
class RateLimitState(BaseModel):
    ...
    requests_this_minute: int = Field(default=0, description="Requests in current minute")
    ...
    hour_reset_at: Optional[datetime] = Field(default=None, description="Hour window reset time")
    day_reset_at: Optional[datetime] = Field(default=None, description="Day window reset time")
...
class RateLimiter:
    ...
    def _calculate_wait_time(self) -> float:
        ...
        return max(min(wait_times), 0.1) if wait_times else self.config.retry_after_seconds
...
class RequestQueue:
    ...
    async def enqueue(
        ...
        async with self._lock:
            ...
            self._queue = deque(
                sorted(self._queue, key=lambda x: x[0], reverse=True)
            )
