...
class BridgeStatus(BaseModel):
    ...
    qr_code: Optional[str] = Field(None, description="QR code for login if not logged in")
...
class MatrixWhatsAppClient:
    ...
    async def send_bridge_command(self, command: str, wait_for_response: bool = True) -> Optional[Dict[str, Any]]:
        """
        ...
        for attempt in range(5):
            ...
            logger.debug(f"Got {len(messages)} messages from room (attempt {attempt + 1})")
            ...
            for msg in messages:
                ...
                if sender == self.bridge_bot and body != command:
                    if is_login_command:
                        ...
                        skip_patterns = ["scan the qr", "not logged in", "you're not logged in"]
    ...
    async def _get_or_create_bridge_room(self) -> str:
        ...
        if self._management_room_id:
            logger.debug(f"Validating cached management room: {self._management_room_id}")
            try:
                ...
                if self._management_room_id in rooms:
                    ...
                    logger.warning(f"Cached room {self._management_room_id} not found, clearing cache")
    ...
    async def login_whatsapp(self) -> Optional[str]:
        ...
        if body and len(body) > 20 and not body.startswith("Hello") and "not logged in" not in body.lower():
            return body
    ...
    async def _send_phone_login_commands(self, management_room: str, phone_number: str) -> None:
        """
        ...
        try:
            ...
            raise ConnectionError(f"Failed to communicate with WhatsApp bridge: {str(e)}")
    ...
    async def _poll_for_pairing_code(self, management_room: str) -> str:
        ...
        for attempt in range(10):
            ...
            for msg in messages:
                ...
                if any(phrase in body.lower() for phrase in ["notification", "check your phone", "sent to your whatsapp"]):
        ...
        error_msg = "Failed to get pairing code from bridge after 10 attempts (~35 seconds)."
        if last_bridge_response:
            error_msg += f"\n\nLast bridge response: {last_bridge_response[:200]}"
        error_msg += "\n\nPlease try again or check bridge logs."

        logger.error(f"Pairing code extraction failed. Last response: {last_bridge_response}")
...
class WhatsAppConnector(BaseConnector):
        ...
        if room_id:
            ...
            for room_info in whatsapp_rooms:
                try:
                    ...
                    logger.warning(f"Error fetching messages from {room_info['room_id']}: {e}")
    ...
    async def sync_all_chats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                contacts_response = await client.send_bridge_command("!wa sync contacts-with-avatars")
            ...
            try:
                appstate_response = await client.send_bridge_command("!wa sync appstate")
            ...
            for room_info in rooms[:5]:  # Limit to first 5 rooms to avoid overwhelming
                try:
                    ...
                    logger.warning(f"Failed to backfill room {room_info.get('room_id')}: {e}")
    ...
    async def sync_bridge(self, timeout: int = 30, force_reconnect: bool = False) -> bool:
        """
