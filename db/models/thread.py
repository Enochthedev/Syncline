...
class Thread(Base):
    ...
    platform = Column(String(50), nullable=False)  # Use String instead of Enum for flexibility
    platform_thread_id = Column(String(255), nullable=False)
    
    # Thread metadata
    title = Column(String(500))
    participants = Column(ARRAY(UUID(as_uuid=True)), nullable=False, default=list)
    ...
    __table_args__ = (
        UniqueConstraint('platform', 'platform_thread_id', name='uq_platform_thread'),
        Index('ix_threads_platform', 'platform'),
